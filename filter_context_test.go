...
func TestContextFilterRunsInAutoescapedTags(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			source:  `{% cycle value fallback as item %}|{{ item }}|{% cycle item %}|{{ item }}`,
			context: pongo2.Context{"value": "one", "fallback": "two"},
			want:    "ctx:one|ctx:one|ctx:two|ctx:two",
			calls:   4,
		},
		{
			name:    "silent referenced cycle string",
			source:  `{% cycle value fallback as item silent %}{{ item }}|{% cycle item %}{{ item }}`,
			context: pongo2.Context{"value": "one", "fallback": "two"},
			want:    "ctx:one|ctx:two",
			calls:   2,
		},
		{
			name:    "stored cycle data",
			source:  `{% cycle value as item %}|{{ item }}`,
			context: pongo2.Context{"value": "one"},
			want:    "ctx:one|ctx:one",
			calls:   2,
		},
		{
			name:    "silent stored cycle data",
			source:  `{% cycle value as item silent %}{{ item }}`,
			context: pongo2.Context{"value": "one"},
			want:    "ctx:one",
			calls:   1,
		},
		{
			name:    "stored cycle literal",
			source:  `{% cycle "one" as item %}|{{ item }}`,
			context: pongo2.Context{},
			want:    "ctx:one|ctx:one",
			calls:   2,
		},
		{
			name:    "silent stored cycle literal",
			source:  `{% cycle "one" as item silent %}{{ item }}`,
			context: pongo2.Context{},
			want:    "ctx:one",
			calls:   1,
		},
		...
		{
			name:    "cycle safe value",
			source:  `{% cycle value %}`,
			context: pongo2.Context{"value": pongo2.AsSafeValue("<b>safe</b>")},
			want:    "<b>safe</b>",
			calls:   0,
		},
		{
			name:    "stored cycle number",
			source:  `{% cycle value as item %}|{{ item }}`,
			context: pongo2.Context{"value": 7},
			want:    "7|7",
			calls:   0,
		...
		{
			name:    "silent stored cycle number",
			source:  `{% cycle value as item silent %}{{ item }}`,
			context: pongo2.Context{"value": 7},
			want:    "7",
			calls:   0,
		...
		{
			name:    "stored cycle safe value",
			source:  `{% cycle value as item %}|{{ item }}`,
			context: pongo2.Context{"value": pongo2.AsSafeValue("<b>safe</b>")},
			want:    "<b>safe</b>|<b>safe</b>",
			calls:   0,
		},
		{
			name:    "firstof string",
		...
		{
			name:    "firstof explicit safe bypass",
		...
		{
			name:    "stored cycle explicit safe bypass",
			source:  `{% cycle value|safe as item %}|{{ item }}`,
			context: pongo2.Context{"value": "<b>safe</b>"},
			want:    "<b>safe</b>|<b>safe</b>",
			calls:   0,
		},
		{
			name:    "silent stored cycle explicit safe bypass",
			source:  `{% cycle value|safe as item silent %}{{ item }}`,
			context: pongo2.Context{"value": "<b>safe</b>"},
			want:    "<b>safe</b>",
			calls:   0,
		},
